...
- 사용자가 화면을 둘러보며 남긴 나머지 피드백(기획/기능 성격, 코드는 안 건드림) — 아래 "다음에 할 것"에 정리하고 사용자에게 우선순위 확인 요청함.
...
- **지원자 관리** (PRD 3.4 "작성자는 대시보드에서 지원자 확인" — 사실 MVP 범위인데 아직 없음): 모집 작성자가 자기 모집글에 누가 지원했는지 보는 화면, 수락/거절 액션. 지원자 쪽도 결과(수락/거절) 확인 수단 필요.
- **모집 유형 워딩** — "개발자 모집"/"기획자 모집"이 헷갈린다는 피드백. 지금은 "이 모집이 찾는 사람"을 뜻하는데(개발자 모집=개발자를 구함), 다른 표현으로 바꿀지 논의 필요.
